...
function Process(action, start, tick, finish)
    ...
    if not IsEntityDead(PlayerPedId()) or mythic_action.useWhileDead then
        if not isDoingAction then
            ...
            Citizen.CreateThread(function()
                ...
                while isDoingAction do
                    ...
                    if IsControlJustPressed(0, 73) and mythic_action.canCancel then
                        TriggerEvent("sho_progbar:client:cancel")
                    end
            ...
            ESX.ShowNotification('error', 'You already doing an action')
        end
    else
        ESX.ShowNotification('error', 'You can\'t do this while unconcious')
    end
end
                    ...
                    if mythic_action.animation ~= nil then
                        ...
                        if mythic_action.animation.task ~= nil then
                            if (DoesEntityExist(player) and not IsEntityDead(player)) then
                                TaskStartScenarioInPlace(player, mythic_action.animation.task, 0, true)
                                while isDoingAction do
                                    if not IsPedUsingScenario(player, mythic_action.animation.task) then
                                        TaskStartScenarioInPlace(player, mythic_action.animation.task, 0, true)
                                    end
                                    Citizen.Wait(500)
                                end
                        ...
                        elseif mythic_action.animation.animDict ~= nil and mythic_action.animation.anim ~= nil then
                            ...
                            if (DoesEntityExist(player) and not IsEntityDead(player)) then
                                ...
                                while isDoingAction do
                                    if not IsEntityPlayingAnim(player, mythic_action.animation.animDict, mythic_action.animation.anim, 3) then
